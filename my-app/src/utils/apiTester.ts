...
const API_BASE_URL = 'https://a40aade1531b.ngrok-free.app';
...
export const testApiEndpoints = async () => {
  ...
  const results: { endpoint: string; status: number; contentType: string; exists: boolean }[] = [];
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      console.log(`${exists ? '✅' : '❌'} ${endpoint} - Status: ${response.status}, Content-Type: ${contentType}`);
    } catch (error) {
...
export const testEndpoint = async (endpoint: string, method: 'GET' | 'POST' | 'PUT' | 'DELETE' = 'GET', body?: any) => {
  try {
    const response = await fetch(`${API_BASE_URL}${endpoint}`, {
      method,
      headers: {
